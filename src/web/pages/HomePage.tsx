...
import { formatVndCompact, todayLocalIso, daysUntil } from '../lib/format';
...
/** Next cookable slot for today: prefers the upcoming un-cooked meal with a recipe. */
function findTodayMeal(plan: MealPlan | null | undefined): MealSlotItem | null {
  if (!plan) return null;
  const today = todayLocalIso();
  const day = plan.days.find((d) => d.date === today);
  if (!day) return null;
  const order: MealSlotItem['slotType'][] = ['dinner', 'lunch', 'breakfast'];
  const cookable = day.slots.filter(
    (s) => s.recipe && (s.status === 'PLANNED' || s.status === 'LEFTOVER')
  );
  for (const slotType of order) {
    const slot = cookable.find((s) => s.slotType === slotType);
    if (slot) return slot;
  }
  return null;
}
...
export const HomePage: React.FC = () => {
  ...
  const todayMeal = findTodayMeal(weekPlan);
          ...
          <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-[#0F3D2E] via-[#14532D] to-emerald-700 shadow-elevated">
            ...
            <div className="relative p-5 space-y-2">
              ...
              <h3 className="font-heading font-extrabold text-lg text-white leading-snug">
                {weekPlan ? 'Hôm nay chưa có món nào được lên lịch' : 'Chưa có thực đơn tuần này'}
              </h3>
              <p className="text-xs text-emerald-100/80">
                {weekPlan
                  ? 'Xem thực đơn tuần để thêm món cho hôm nay.'
