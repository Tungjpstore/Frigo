...
export const MealCard: React.FC<MealCardProps> = ({ slot, onClick, onSwapClick }) => {
  ...
  if (slot.status === 'FLEXIBLE') {
    return (
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-emerald-50 border border-emerald-100 text-emerald-700 flex items-center justify-center text-lg shrink-0">
            ✨
          </div>
          <div>
            <div className="flex items-center gap-1.5 mb-0.5">
  ...
  return (
    ...
    >
      {isCooked && (
        <div className="absolute top-2 right-2 px-2 py-0.5 rounded-full bg-emerald-600 text-white text-[10px] font-semibold flex items-center gap-1 shadow-xs">
          ...
          <div className="mt-2 pt-1.5 border-t border-slate-100 flex items-center justify-between text-xs">
            <div className="text-[11px]">
              {slot.availabilityPercent === 100 ? (
                <span className="text-emerald-700 font-bold">Đủ 100% trong tủ</span>
              ) : (
