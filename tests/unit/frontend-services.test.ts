...
function signIn(userId: string, householdId: string) {
  storage.setItem('frigo_token', `token_${userId}`);
...
describe('household-scoped local projections', () => {
  ...
  it('drops a legacy unscoped week plan owned by a different household', async () => {
    ...
    // the leaky legacy key must be gone afterwards
    expect(storage.getItem('frigo_active_meal_plan')).toBeNull();
  });

  it('migrates a legacy week plan when it belongs to the current household', async () => {
    const http = await import('../../src/web/services/http');
    signIn('user-a', 'house-a');
    storage.setItem(
      'frigo_active_meal_plan',
    ...
    expect(storage.getItem('frigo_active_meal_plan')).toBeNull();
    expect(storage.getItem(http.weekPlanCacheKey('house-a'))).toBeTruthy();
  });
});
