...
function normalizePayload(payload: unknown): unknown {
  ...
  const items = Array.isArray(record.items)
    ? record.items.map((item) => {
        ...
        const numberValue = (input: unknown): unknown => {
        ...
        return {
          ...
          estimated_quantity: numberValue(value.estimated_quantity),
          unit_price_vnd: numberValue(value.unit_price_vnd),
          total_price_vnd: numberValue(value.total_price_vnd),
  ...
  return {
    ...
    total_amount_vnd: record.total_amount_vnd === null ? undefined : numberValue(record.total_amount_vnd),
    items,
  };
}
...
export class GroqProvider implements AIProvider {
  ...
  constructor(apiKey: string, baseUrl = DEFAULT_BASE_URL, visionModel = DEFAULT_VISION_MODEL) {
...
Unit chỉ được là g, kg, ml, l, piece, pack, bunch hoặc slice. Nếu không chắc, dùng piece và confidence thấp.`;
    ...
    const validated = VisionScanResultSchema.parse(normalizePayload(parsed));
    if (validated.items.length === 0) throw new Error('Groq returned no detectable ingredients');
...
Unit chỉ được là g, kg, ml, l, piece, pack, bunch hoặc slice. Bỏ qua dòng không phải sản phẩm.`;
    const parsed = await this.completeVision(prompt, params);
    const validated = ReceiptScanResultSchema.parse(normalizePayload(parsed));
    if (validated.items.length === 0) throw new Error('Groq returned no receipt items');
  ...
  private async completeVision(prompt: string, params: VisionScanParams): Promise<unknown> {
    ...
    const response = await fetch(`${this.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.1,
        // Groq free-tier OTPM is currently 1000; leave headroom for this
        // structured response so receipt scans do not get rejected with 429.
    ...
    const payload = (await response.json()) as { choices?: Array<{ message?: { content?: ChatMessageContent } }> };
    const message = payload.choices?.[0]?.message as ({ content?: ChatMessageContent; reasoning?: string } | undefined);
    const rawContent = message?.content || message?.reasoning;
    const parsed = parseJsonObject(contentText(rawContent));
    if (!parsed) throw new Error('Groq returned an empty or non-JSON response');
    return parsed;
  }
...
export { DEFAULT_BASE_URL as GROQ_DEFAULT_BASE_URL, DEFAULT_VISION_MODEL as GROQ_DEFAULT_VISION_MODEL };
