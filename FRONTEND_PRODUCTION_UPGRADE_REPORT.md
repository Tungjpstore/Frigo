...
> Historical source-branch report (`fafe1cc`), not the integrated security contract.
> See `FRONTEND_SECURITY_INTEGRATION_REPORT.md` for current authentication,
> ownership, migration policy, and verification. The original Bearer client and
> household-only legacy-cache adoption described below were not retained.
